            ...
            for weather in weathers:
                ...
                for day in days:
                    ...
                    for catagory in catagories:
                        ...
                        if parkinglot.split('\\')[-1] == dataset:

                            files = [f.path for f in os.scandir(catagory) if '.jpg' in f.name]

                            for file in files:

                                desfilename = os.path.join( output,dataset[:-9], 'train',catagory.split('\\')[-1],ntpath.basename(file))
                            ...
                            for file in files:
                                desfilename = os.path.join(output, dataset[:-9], 'test', catagory.split('\\')[-1],
